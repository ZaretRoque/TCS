...
namespace CincoEnLinea {
    public partial class TableroJugar : Form {
        ...
        public void PintarTiro(int posX, int posY, Bitmap panelPintar, PictureBox pulsado) {
        ...
        public void ComprobarHorizontal(int turno) {
            int auxiliar = -1;
            int contador = 0;
            for(int i =0; i < 8; i++) {
                for(int j=0; j < 8; j++) {
                    if(turno == tableroTiros[i, j] && j-1 == auxiliar) {
                        contador++;
                    }
                    if (contador == 5) {
        ...
        public void ComprobarVertical(int turno) {
            int auxiliar = -1;
            int contador = 0;
            for (int i = 0; i <8; i++) {
                for (int j = 0; j <8; j++) {
                    if (turno == tableroTiros[j,i] && i-1 == auxiliar) {
                        contador++;
                    }
                    if (contador == 5) {
                        MessageBox.Show("ganaste", "Yei", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
